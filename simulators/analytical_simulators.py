...
# created by Ao Qu
class StepFunction(BaseSimulator):

    def __init__(self, n_points=2001, noise_sigma=0.10, n_inputs=1, n_outputs=1):
        super().__init__(n_points, noise_sigma, n_inputs, n_outputs)
        self.n_inputs = 1
        self.n_outputs = 1

    def mean(self, x): 
        return np.where(x < 0, -100, 100)

    def stddev(self, x): 
        return self.noise_sigma * np.ones_like(x)

    def noise(self, x): 
        return np.random.normal(loc=0, scale=1, size=[x.shape[0], self.n_outputs])

    def search_space(self): 
        return np.linspace(-10, 10, self.n_points).reshape(-1, 1)